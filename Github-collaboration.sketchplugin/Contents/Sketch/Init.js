...
exports.default = function (context) {
  ...
  (0, _common.executeSafely)(context, function () {
    ...
    if (currentFileName) {
      (0, _analytics.sendEvent)(context, 'Init', 'Start init');
      var command = 'git init && git add "' + currentFileName + '"';
      var message = (0, _common.exec)(context, command);
      context.document.showMessage(message);
      var remoteURL = (0, _common.createInput)(context, 'URL of the remote repo (you can create one here: https://github.com/new)', 'Add remote', 'Not now');

      if (remoteURL.responseCode == 1000 && remoteURL.message != null) {
        (0, _analytics.sendEvent)(context, 'Init', 'Add remote');
        command = 'git remote add origin ' + remoteURL.message + '; exit';
        message = (0, _common.exec)(context, command);
        context.document.showMessage(message.split('\n').join(' '));
      } else {
        (0, _analytics.sendEvent)(context, 'Init', 'Cancel add remote');
      }
    } else {
      (0, _common.createFailAlert)(context, 'Failed...', 'Cannot get the current file name');
    }
  });
};
...
module.exports = exports['default']; // Init git repo and add current file to the repo (cmd alt ctrl n)
